...
// ── Config ───────────────────────────────────────────────────
export const CONFIG = {
  ADMIN_WHATSAPP: "918256913154",        // Admin WhatsApp number
  GOOGLE_SCRIPT_URL: "https://script.google.com/macros/s/AKfycbzxRR6Etd9kZ4zPY-ErJnPohdKggDMKcMUrVuy3mN7_kTeZq7B1uUmFxHl9uZOtfm8VuA/exec", //
  MAX_IMAGE_SIZE_MB: 1,
  MAX_IMAGES: 3,
...
export async function submitToGoogleSheets(data) {
  // Use GET + URL params — most reliable method for Apps Script from browser
  // No CORS issues, no preflight, no no-cors silent failures
  try {
    // Build query string from data object
    const params = new URLSearchParams();
    Object.entries(data).forEach(([key, val]) => {
      if (val !== null && val !== undefined) {
        // Arrays (like images) joined as comma string
    ...
    // Use no-cors GET — Apps Script still receives and processes it
    await fetch(url, { method: "GET", mode: "no-cors" });

    console.log("✅ Sent to Google Sheets via GET");
    return true;
  } catch (err) {
